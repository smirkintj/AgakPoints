...
import { Award, Check, CheckCircle2, Clock, FileText } from "lucide-react";
...
export function SessionSummaryModal({
  ...
  const [jiraState, setJiraState] = useState<"idle" | "loading" | "done">("idle");
  const [jiraResults, setJiraResults] = useState<JiraTicketResult[]>([]);
  ...
  const totalActions = jiraResults.reduce((s, r) => {
    let n = 2; // sp + comment always
    if (!r.assignee.skipped) n++;
    return s + n;
  }, 0);
  const succeededActions = jiraResults.reduce((s, r) => {
    let n = (r.sp.ok ? 1 : 0) + (r.comment.ok ? 1 : 0);
    if (!r.assignee.skipped && r.assignee.ok) n++;
    return s + n;
  }, 0);
      ...
      >
                  ...
                  <div key={t.id} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-white/3 border border-white/8">
                    ...
                    {jiraState === "loading" && (
                      <div className="flex items-center gap-1 shrink-0">
                        <span className="w-10 h-4 rounded bg-white/10 animate-pulse" />
                      </div>
                    )}
                    {jiraState === "done" && (() => {
                      const r = jiraResults.find(x => x.ticketId === t.id);
                      if (!r) return null;
                      return (
                        <div className="flex items-center gap-1 shrink-0">
        ...
        <div className="px-8 py-4 border-t border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {jiraState === "loading" && <span className="text-xs text-white/40 flex items-center gap-1"><Clock className="w-3 h-3 animate-spin" /> Posting to JIRA…</span>}
            {jiraState === "done" && (
              <span className={`text-xs flex items-center gap-1 ${succeededActions === totalActions ? "text-emerald-400" : "text-amber-400"}`}>
                <CheckCircle2 className="w-3.5 h-3.5" />
                {succeededActions}/{totalActions} actions succeeded
              </span>
            )}
          </div>
